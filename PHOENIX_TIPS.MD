...
#### Контроллер для страницы комментариев.
...
Если поменять структуру возвращаемую join на `{:ok, %{hey: "there"}, socket}`, в будет видно, что она
...
id для каждого подключения уже известен, он передается в ф-ю join. Что бы "протащить" его в handle_in можно использовать
...
Когда на клиенте вызывается конструктор и создается websocket, на сервере срабатывает коллбэк connect в [user_socket.ex](web/channels/user_socket.ex).
...
# 144
## Получение данных пользователя. Вложенные (nested) модели.
Что бы отобразить пользователей оставивших комментарии, нужно в функции join подгрузить связанную с каждым комментарием модель User.
Это можно было бы сделать по аналогии с комментариями, функцией Repo.preload. 
Что бы preload загрузил данные для User, нужно использовать параметр указывающий на вложенную структуру: `Repo.preload(:comments)`
нужно заменить на `Repo.preload(comments: [:user])`, где указывает, что для каждой вложенной записи в comments, нужно
получить вложенную запись user.\
Если сейчас обновить страницу, в консоли браузера будет видно, что поле user в комментариях не заполнено, это произошло
т.к. для модели [Comment](web/models/comment.ex) была указана директива сериализации `@derive {Poison.Encoder...`, игнорирующая все поля записи кроме "content".\
Нужно изменить код на `@derive {Poison.Encoder, only: [:content, :user]}`. 
Для модели [User](web/models/user.ex) так же нужно указать `@derive {Poison.Encoder, only: [:email]}`.\
Возможно потребуется рестарт phoenix.\
Теперь в консоли будет список комментариев, имеющих поле user, содержащее email пользователя.  

# 145
## Отображение пользователей оставивших комментарий
Что бы отобразить email пользоватея рядом с комментарием, достаточно модифицировать функцию commentTemplate в [socket.js](web/static/js/socket.js).
После обновления страницы рядом с комментарием будет отображен email пользователя.\
Сейчас пользователи будут отображаться только для комментариев загруженных в join. Комментарии добавляемые интерактивно,
через handle_in будут отображатсья с email "undefined". Это происходит из-за "хака" использованного на шаге 143.
Для исправления нужно после вставки выполнить `Repo.preload(comment, :user)` для ченьжсета полученного из Repo.insert.\
Теперь email будет отображатсья при добавлении комментария.  

# FIN
На этом обучающая часть завершается :)

# TODO
- Добавить отображение identicon в качестве аватарки
- Ознакомится с констекстами в новой версии Phoenix (с 1.3)
- Перенести проект на актуальную версию Phoenix и сопутствующих библиотек (Ecto, Poison и т.п....)
